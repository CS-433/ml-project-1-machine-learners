...
def drop_features(x_tr, x_te, feature_names, feat_indexes, features_to_drop):
    remaining_features = [
        feature for feature in feature_names if feature not in features_to_drop
    ]
...
def drop_useless_features(x_train, x_test, feature_names, feat_indexes):
    ...
    x_train, x_test, feature_names, feat_indexes = drop_features(
        x_train, x_test, feature_names, feat_indexes, features_to_drop=variables_to_drop
    )
...
def replace_weird_values(x_train, x_test, feature_names, feat_indexes):
    ...
    value_replacements = {
        ...
        for feature, dic in value_replacements.items()
    }
...
def one_hot_categoricals(x_train, x_test, feature_names, feat_indexes, feature_types):
    print("Pipeline Stage 7 - One Hot Encoding Categoricals...")
    categorical_features = [
        feature
        for feature in feature_names
        if feature_types[feature] == feature_types.FeatureType.CATEGORICAL
    ]
    ...
    non_cat_indexes = [
        index
        for feat, index in feat_indexes.items()
...
def write_datasets(x_train, x_test, y_train, feature_names):
    print("Pipeline Stage 10 - Writing Datasets...")
