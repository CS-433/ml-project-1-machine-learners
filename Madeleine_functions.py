...
def oversampling(x,y,proportion):
    ...
    resample_id = np.random.choice(class_1_id, size=int(len(class_0_id)*proportion), replace=True)
...
def undersampling(x,y,prop):
    ...
    class_0_id = np.where(y == 0)[0]
    class_1_id = np.where(y == 1)[0]
    resample_id = np.random.choice(class_0_id, size=int((1+prop)*len(class_1_id)), replace=False)
